...
class PandaGripperBase(GripperModel):
    ...
    def __init__(self, idn=0):
        # prefer the canonical gripper XML shipped in the repo at `envs/assets/grippers`
        repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', '..'))
        xml_path = os.path.join(repo_root, 'envs', 'assets', 'grippers', 'panda_gripper.xml')
        if not os.path.exists(xml_path):
            # fallback to the local package assets if repo copy is not present
...
class PandaVirtualGripper(PandaGripperBase):
    """
...
class PandaGripperNarrowBase(GripperModel):
    ...
    def __init__(self, idn=0):
        repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', '..'))
        xml_path = os.path.join(repo_root, 'envs', 'assets', 'grippers', 'panda_gripper_narrow.xml')
        if not os.path.exists(xml_path):
